...
import casia.isi.elasticsearch.common.Scripting;
...
public class EsIndexSearchImp {
	 ...
	 * 日志对象
	 */
	private static Logger logger = Logger.getLogger(EsIndexSearchImp.class);
	 ...
	 * @param indexName
	 * 			索引名称，允许多个索引，索引间用逗号隔开，格式 indexName1,indexName2
	 * @param typeName
	 * 			索引下的类型名称，允许多个类型，用逗号隔开,格式typeName1,typeName2
	 */
	 ...
	 *            字段
	 * @param term
	 *            字段对应的值，只能转入一个，且不能有有空格.如果传入的值为负值，请用双引号包起来，例如：<br>
...
//		this.queryFilterMustJarr.add( existsJson );
	 ...
	 * 
	 * @param fields
	 * 			索引查询后要返回值的字段，只有建索引时，有存储的字段此处才可能有返回值，对于只索引不存储的字段，此处得不到返回值
	 * @return
	 */
	private String getQueryString(String[] fields) {
		this.fields = fields;
...
//		}
	 ...
	 *            统计的字段
	 * @param topN
	 *            要求返回的结果数 ,topN 等于 0 时，将返回所有的统计结果 。topN 小于0时，不返回结果。
	 ...
	 * @return 前topN个结果的list ， 每一项为一个数组， 第一项为统计字段，第二项为字段等于该值的文档数，第三项为字段为详细数据  。
	 * 		    注意 ： 执行 getTotal()方法               为条件过滤后未执行聚合分组的文档总量；
	 *			    执行 getCountTotal()方法               为条件过滤后执行聚合总量；
	 * 			       
	 */
	 ...
	 * @param field
	 *            统计的字段
	 * @return Map<String, Long> ， 每一项为一个字段聚合值， 第一项为聚合字段，第二项为该字段聚合数 。
	 * 		    注意 ： 执行 getTotal()方法               为条件过滤后未执行聚合分组的文档总量；
	 */
	 ...
	 * @return List<String[]>  String[0]为时间， String[1]为统计的数量
	 */
...
//		addRangeTerms(field, startTime, endTime);
	 ...
	 * @param format 时间格式 例如：yyyy-MM-dd 
	 * @param interval 粒度 (1M代表每月，1d代表每日，1H代表每小时)
	 * @param childField 要聚合的字段
	 * @return List<String[]>  String[0]为时间， String[1]为统计的文档数量 ， String[2]为childField的统计数
	 */
	 ...
	 * @return List<String[]>  String[0]为时间， String[1]为统计的文档数量 ， String[2-N]为childField字段依次统计数
	 */
	 ...
	 * @param interval 粒度 (1M代表每月，1d代表每日，1H代表每小时)
	 * @param childField 要聚合的字段
	 * @return List<String[]>  String[0]为时间; String[1]为统计的文档数量 ; String[2]为childField的不同类型的个数（json格式）例如:[{"doc_count":112,"key":1},{"doc_count":12,"key":2}]
	 */
	 ...
	 * @return List<String[]>  String[0]为时间; String[1]为统计的文档数量 ; String[2]为时间段内的N条数据（json格式）例如:[{"uid":"w4x08z5"},{"uid":"z089903xzy"}]
	 */
